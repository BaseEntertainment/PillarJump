...
namespace I2.Loc
{
	public partial class LocalizationEditor
	{
		...
		void OnGUI_Keys_ListSelection(int KeyListFilterID)
		{
			...
			// Terms Filter
			{
				KeyList_Filter = EditorGUILayout.TextField(KeyList_Filter, GUIStyle.none, GUILayout.ExpandWidth(true));
				GUILayout.Label("", GUILayout.ExpandWidth(true));
				mKeyListFilterRect = GUILayoutUtility.GetLastRect();
				mKeyListFilterRect.xMax += 4;

				KeyList_Filter = GUITools.TextField(mKeyListFilterRect, KeyList_Filter, 255, GUIStyle.none, KeyListFilterID);
			}



			if (GUILayout.Button(string.Empty, GUIStyle.none, GUILayout.ExpandWidth(false)))
			{
