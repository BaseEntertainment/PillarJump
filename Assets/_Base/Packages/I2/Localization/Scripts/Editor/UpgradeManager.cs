	...
	public class UpgradeManager
	{
		...
		public static void EnablePlugins(bool bForce = false)
		{
			...
			foreach (NamedBuildTarget target in buildTargets)
			{
				...
				{
#if UNITY_5_6
						if (target == NamedBuildTarget.NintendoSwitch) continue;    // some releases of 5.6 defined BuildTargetGroup.Switch but didn't handled it correctly
#endif
			...
			EnablePluginsOnPlatform(NamedBuildTarget.iOS);
		}

		static void EnablePluginsOnPlatform(NamedBuildTarget buildTarget)
		{
			string Settings = PlayerSettings.GetScriptingDefineSymbols(buildTarget);
			...
			if (HasChanged)
			{
				try
				{
					...
					PlayerSettings.SetScriptingDefineSymbols(buildTarget, Settings);
				}
				catch (Exception)
				{
				}
